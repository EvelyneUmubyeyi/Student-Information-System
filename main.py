...
def create_student(student_email, student_name, gender, date_of_birth, address, phone_number, date_of_enrollment, year, major):
...
def add(link,index,school_name,programs):
...
def upload_outlines(school_name,index,programs):
    ...
    next = tk.Button(frame, text="Next", command=lambda: add(value.get(), index,school_name,programs), font="Calibri",
                     bg="#000000", fg="white", height=1, width=17)
...
def view_outline():
    ...
    warning = tk.Label(frame, text="If you can not see anything, it means that no outline was added.",
                       font=("Calibri", 12))
    ...
    try:
        with open("degree_programs.csv", 'r') as programs_records:
            ...
            for program in read_program:
                program_name =tk.Label(frame, text=program[1], font="Raleway")
...
def visualize():
    ...
    for key in labels_list:
        ...
        data_points.append(labels_list[key])
...
def display_menu(user_type,student_email=""):
    ...
    if user_type == "admin":
        ...
        visualize_btn = tk.Button(frame, text="visualize", command=lambda: visualize(),
                                 font="Calibri", bg="#000000", fg="white",
                                 height=1, width=25)
...
#submit button
submit_btn = tk.Button(frame, text = "Log In", command=lambda:authenticate("login"), font="Raleway",bg = "#000000", fg="white",height=1, width=15)
